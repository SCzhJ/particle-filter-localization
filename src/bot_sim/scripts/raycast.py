...
class Raycast:
    ...
    def OccupancyCheck(self,x,y):
        ...
        if x >= self.width or y >= self.height:
            # print("hit map boundary")
            return True
        # check occupancy
        occ = self.gridMap.data[y*self.width+x]
        # -1 is grid with no info
        if occ > self.occupancyThreshold or occ == -1:
            # print("occ",occ)
            return True
        else:
            # print("free")
    ...
    def dummyRanges(self):
        self.rays = []
        for r in range(len(self.scan.ranges)):
            self.rays.append(4.0)
        self.rays[0] = 1.0
        self.rays[-1] = 7.0
