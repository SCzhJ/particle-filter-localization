...
if __name__ == '__main__':
    while True:
        ...
        if is_node_alive('map_server'):
            ...
            subprocess.Popen(['roslaunch', 'bot_sim', 'map_server.launch'])
            time.sleep(1)

        if is_node_alive('imu_filter'):
            pass
        else:
            subprocess.Popen(['roslaunch', 'bot_sim', 'imu_filter.launch'])
            time.sleep(1)
        ...
        if is_node_alive('threeD_lidar_merge_pointcloud_test'):
            ...
            subprocess.Popen(['roslaunch', 'bot_sim', 'lidar_merge_pointcloud_test.launch'])
            time.sleep(1)

        if is_node_alive('hdl_localization_nodelet'):
            pass
        else:
            subprocess.Popen(['roslaunch', 'hdl_localization', 'hdl_localization.launch'])
            time.sleep(1)
