    ...
    public:
        ...
        class Node{
            public:
                ...
                double count_edge_value(){
                    if(isObstacle)return 3;
                    return 1;
                }
                Node(int x, int y){
                    dis_to_goal = INF + 1;
...
void dstarlite::when_receive_new_goal(geometry_msgs::PointStamped::ConstPtr goal_pose_msg, double real_start_x, double real_start_y){
    ...
    reset();
    dstar_list.clear();
    while(!changed_obstacle_nodes.empty())changed_obstacle_nodes.pop();
...
void dstarlite::dstar_update_node(Nodeptr cur){
    ...
    for(int i = 0; i < 8; i++){
        ...
        int ny = cur->y + dy[i];
        if(nx < 0 || nx >= max_x || ny < 0 || ny >= max_y)continue;
        double value = cur->count_edge_value()*map[nx][ny]->count_edge_value();
    ...
    for(int i = 0; i < 8; i++){
        ...
        if((map[nx][ny]->succ == cur || cur->succ == map[nx][ny]) && map[nx][ny]->dis_to_goal == map[nx][ny]->rhs && cur->dis_to_goal == cur->rhs)lct->link(map[nx][ny],cur);
    }
    // if(previous_rhs != cur->rhs){
    //     if(previous_rhs == cur->dis_to_goal){
    //         for(int i = 0; i < 8; i++){
    //             int nx = cur->x + dx[i];
    //             int ny = cur->y + dy[i];
    //             if(nx < 0 || nx >= max_x || ny < 0 || ny >= max_y)continue;
    //             if(map[nx][ny]->succ == cur && map[nx][ny]->dis_to_goal == map[nx][ny]->rhs)lct->del(cur, map[nx][ny]);
    //         }
    //     }
    ...
    //             int ny = cur->y + dy[i];
    //             if(nx < 0 || nx >= max_x || ny < 0 || ny >= max_y)continue;
    //             if(map[nx][ny]->succ == cur && map[nx][ny]->dis_to_goal == map[nx][ny]->rhs)lct->link(cur, map[nx][ny]);
    //         }
    //     }
    // }
    // //deal with previous and rhs
    // if(previous_succ == cur->succ){
    //     if(cur->succ != nullptr && cur->succ->dis_to_goal == cur->succ->rhs && cur -> rhs == cur -> dis_to_goal && previous_rhs != cur -> dis_to_goal)lct->link(cur, cur->succ);
    //     if(cur->succ != nullptr && cur->succ->dis_to_goal == cur->succ->rhs && cur -> rhs != cur -> dis_to_goal && previous_rhs == cur -> dis_to_goal)lct->del(cur, cur->succ);
    // }
    // else{
    //     if(previous_succ != nullptr && previous_succ->dis_to_goal == previous_succ->rhs && previous_rhs == cur -> dis_to_goal)lct->del(cur, previous_succ);
    //     if(cur->succ != nullptr && cur->succ->dis_to_goal == cur->succ->rhs && cur -> rhs == cur -> dis_to_goal)lct->link(cur, cur->succ);
    // }
    // //deal with succ
    // if(previous_list_status == IN_LIST || cur -> rhs != previous_rhs){    
    cur->dstar_list_status = IN_LIST;
    dstar_list.insert(cur);
    // }
}
...
void dstarlite::publish_all_map_status(ros::Publisher& pub){
    ...
    for(int i = 0; i < max_x; i++){
        for(int j = 0; j < max_y; j++){
            ...
            if(map[i][j]->last_out_list_round == round)all_map_status.data[index] = 100;
...
void dstarlite::try_to_find_path(){
    ...
    while(cur!=nullptr && cur!=final_goal_node){
        if(cur->used_for_find_path == for_find_path_round){
            ROS_INFO("Loop");
            return;
        }
        if(cur->succ == nullptr || lct->find_root(cur) != lct->find_root(cur->succ)){
...
double dstarlite::calculate_velocity(Nodeptr cur){
    int obstacle_num = cur->isObstacle + cur->succ->isObstacle;
    if(obstacle_num == 0)return velocity;
    if(obstacle_num == 1)return velocity * 0.5;
