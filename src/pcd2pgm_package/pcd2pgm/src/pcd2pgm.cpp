...
int main(int argc, char **argv) {
  ...
  private_nh.param("max_accept_delta_z", max_accept_delta_z, 0.1);
...
void SetMapTopicMsg(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  //设置栅格地图大小
  msg.info.width = ceil((x_max - x_min) / map_resolution);
  msg.info.height = ceil((y_max - y_min) / map_resolution);
  //实际地图中某点坐标为(x,y)，对应栅格地图中坐标为[x*map.info.width+y]
  std::vector<double> temp_map_data;
  temp_map_data.resize(msg.info.width * msg.info.height);
  temp_map_data.assign(msg.info.width * msg.info.height, -114514);
  ...
  for (int iter = 0; iter < cloud->points.size(); iter++) {
    int i = round((cloud->points[iter].x - x_min) / map_resolution);
    if (i < 0 || i >= msg.info.width)
      continue;

    int j = round((cloud->points[iter].y - y_min) / map_resolution);
    if (j < 0 || j >= msg.info.height - 1)
      continue;
    // 栅格地图的占有概率[0,100]，这里设置为占据
    // msg.data[i + j * msg.info.width] = cloud->points[iter].z*40;
    temp_map_data[i + j * msg.info.width] = std::max(temp_map_data[i + j * msg.info.width], static_cast<double>(cloud->points[iter].z));
    //    msg.data[i + j * msg.info.width] = int(255 * (cloud->points[iter].z *
    //    k_line + b_line)) % 255;
  }
  for(int i=0;i<msg.info.width;i++){
    for(int j=0;j<msg.info.height;j++){
      if(temp_map_data[i+j*msg.info.width] == -114514)continue;
      double delta_z = 0;
      int num = 0;
      for(int di=-2;di<=2;di++){
        for(int dj=-2;dj<=2;dj++){
          if(i+di>=0 && i+di<msg.info.width && j+dj>=0 && j+dj<msg.info.height){
            if( temp_map_data[i+di+(j+dj)*msg.info.width] == -114514)continue;
            delta_z += temp_map_data[i+j*msg.info.width] - temp_map_data[i+di+(j+dj)*msg.info.width];
            num++;
          }
        }
      }
      delta_z /= num;
      if(abs(delta_z) > max_accept_delta_z){
